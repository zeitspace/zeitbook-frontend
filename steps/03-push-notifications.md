...
> We'll implement push notifications using the Web Push API, which is supported by Chrome, Firefox, and Opera. It's also in development for Edge. Safari doesn't implement this API, but a Safari-specific notifications API does exist. The Web Push API isn't supported at all by IE.
